...
const todoSlice = createSlice({
  ...
  reducers: {
    ...
    removeTodo: (
      state: TodosState,
      { payload: { id, status } }: PayloadAction<Todo>
    ) => {
    ...
    reorderTodos: (
      ...
      {
        payload: { todos, type },
      }: PayloadAction<{ todos: Todo[]; type: TodoStatus }>
    ) => {
    ...
    crossColumnReorder: (
      ...
      if (destinationStatus === TodoStatus.InProgress)
        state.inProgress = destination;
      if (destinationStatus === TodoStatus.Completed)
